...
Du théorème ci-dessus, on peut établir les faits suivants pour une
...
Le théorème ne dit pas que les valeurs de la diagonale $a_{11}, \dots,
a_{nn}$ sont les valeurs propres de la matrice $\mat{A}$ en général,
mais bien seulement que
\begin{itemize}
\item $\sum_{i = 1}^n a_{ii} = \text{somme des valeurs propres}$; et
  que
\item $\prod_{i = 1}^n a_{ii} = \text{produit des valeurs propres}$.
\end{itemize}

Par ailleurs, si un élément de la diagonale d'une matrice triangulaire
est nul, alors
\begin{itemize}
...
\item le déterminant de la matrice est nul et la matrice est
  singulière.
\end{itemize}
Cette observation nous permet d'ajouter un énoncé au
...
\begin{rem}
  L'équation cractéristique $\det(\lambda \mat{I} - \mat{A})$ peut
