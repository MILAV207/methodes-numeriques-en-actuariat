...
différentielles, mécanique quantique, finance, \capsule{moteurs de
  recherche}, etc.
...
de lui-même; voir la figure~\ref{fig:valeurspropres:illustration}.
...
\begin{exemple}
  ...
  Cela dit, le calcul de l'exponentielle de $\mat{A}$ est simple si la
  matriceest diagonale. Supposons, sans perte de généralité, que
  $\mat{A}$ est une matrice $2 \times 2$:
  \begin{equation*}
    \mat{A} =
    \begin{bmatrix}
      a_1 & 0 \\ 0 & a_2
  ...
  à prendre l'exponentielle des éléments de la diagonale.

  Poussons l'idée une étape plus loin. Supposons que $\mat{B}$ est une
  ...
  \end{align*}
  Si l'on peut écrire une matrice $\mat{B}$ sous la forme $\mat{B} =
  \mat{P^{-1}} \mat{A} \mat{P}$, alors le calcul de son exponentielle
  sera simple.%
  \qed
\end{exemple}
