...
il existe quelques applications intéressantes de la conversion de et vers
des bases générales, comme nous le verrons.
...
\item On a un ensemble fini de nombres pour représenter $\mathbb{R}$.
  ...
  \begin{align*}
    ...
    &= x_{\text{max}} - 2^{971}.
  \end{align*}
  Les deux plus petits nombres représentables sont donc distants de
  $2^{-\nombre{1024}} \approx 10^{-324}$, alors que les deux plus
  grands le sont de $2^{971} \approx 10^{292}$!
\end{enumerate}
