...
\begin{prob-enonce}
  En actuariat, et particulièrement dans les examens d'organismes
  comme la SOA et la CAS, l'utilisation de la calculatrice de type
  TI-30XS MultiView est très répandue.
  
  La plupart du temps, cet outil retourne le résultat attendu. Par
  exemple, si on vérifie que l'équation $(x-1)(x+1) - x^2 + 1$
  équivaut bel et bien à $0$ pour une valeur de $x$ donnée, le
  résultat est satisfaisant :
  
  \begin{enumerate}
    \item Assigner la valeur $1 \times 10 ^ 6$ à la variable $x$.
    \item Effectuer l'opération $(x-1)(x+1) - x^2 + 1$.
    \item Le résultat est $0$.
  \end{enumerate}
  
  Sans surprise, le résultat est $0$. Or, un deuxième
  test avec une valeur de $x$ plus élevée pourrait permettre
  de remettre en question la qualité de ce modèle de
  calculatrice :
  
  \begin{enumerate}
    \item Assigner la valeur $1 \times 10 ^ 8$ à la variable $x$.
    \item Effectuer l'opération $(x-1)(x+1) - x^2 + 1$.
    \item Le résultat est $1$.
  \end{enumerate}
  
  Le résultat de $1$ peut paraître surprenant, mais permet-il
  vraiment de douter de la qualité de la calculatrice?
\end{prob-enonce}
...
Ce chapitre se penche justement sur les grands principes de l'%
...
\begin{prob-astuce}
  Nous pourrons utiliser la représentation simplifiée
  \eqref{eq:ordinateurs:fl(x)} afin d'illustrer les calculs de la
  calculatrice à chaque étape des calculs présentés dans l'énoncé.
  Toutefois, la calculatrice garde 13 chiffres significatifs en mémoire,
  alors nous devrons modifier l'élément $F$ de la notation pour qu'il
  contienne 13 décimales.
\end{prob-astuce}
...
\begin{prob-solution}
  ...
  Le premier calcul s'effectue de la façon suivante : 
  ...
  \begin{enumerate}
    ...
    \item Effectuer l'opération $(x-1)(x+1) - x^2 + 1$.
      ...
      \begin{align*}
        (&fl(x) - 0,0000001000000 \times 10^7 )(fl(x) + 0,0000001000000 \times 10^7 )\\
          &\qquad - fl(x)^2 + 0,1000000000000 \times 10^1\\
        &= (0,0999999000000 \times 10^7)(0,1000001000000 \times 10^7)\\
          &\qquad - 0,1000000000000 \times 10^{13} + 0,1000000000000 \times 10^1\\
        &= 0,0999999999999 \times 10^{13} - 0,1000000000000 \times 10^{13}\\
          &\qquad + 0,1000000000000 \times 10^1\\
        &= - 0,0000000000001 \times 10^{13} + 0,1000000000000 \times 10^1\\
  ...
  Dans ces opérations, le nombre de chiffres significatifs nécessaire
  ne dépasse jamais la capacité de 13 de la calculatrice.
  
  Le deuxième calcul s'effectue de la façon suivante : 
  ...
  \begin{enumerate}
    ...
    \item Effectuer l'opération $(x-1)(x+1) - x^2 + 1$.
      ...
      \begin{align*}
        (&fl(x) - 0,0000000010000 \times 10^9 )(fl(x) + 0,0000000010000 \times 10^9 )\\
          &\qquad - fl(x)^2 + 0,1000000000000 \times 10^1\\
        &= (0,0999999990000 \times 10^9)(0,1000000010000 \times 10^9)\\
          &\qquad - 0,1000000000000 \times 10^{17} + 0,1000000000000 \times 10^1\\
        &= 0,0\boldsymbol{9999999999999}999 \times 10^{17} - 0,1000000000000 \times 10^{17}\\
          &\qquad + 0,1000000000000 \times 10^1\\
        &= \mathbf{0,1000000000000} \times 10^{17} - 0,1000000000000 \times 10^{17}\\
          &\qquad + 0,1000000000000 \times 10^1\\
        &= 0 + 0,1000000000000 \times 10^1\\
        &= 1
      \end{align*}
    \item Le résultat est $1$.
  \end{enumerate}
  ...
  mathématique est lors du passage de la troisième à la quatrième ligne.
  En effet, les chiffres en gras à la troisième ligne représentent la
  capacité de la calculatrice en nombres significatifs. Or, puisque le
  chiffre complet dépasse cette capacité, la calculatrice doit
  l'arrondir à $0,1000000000000 \times 10^17$.
  
  Étant donné que ces erreurs de calcul touchent \emph{tous} les
  outils de calculs numériques, on peut confirmer que R souffre
  des mêmes maux que la calculatrice :
...
@
  ...
  Afin de prévenir ce genre d'erreurs, il conviendra donc de
  respecter les 6 principes de la
  \autoref{sec:ordinateurs:arithmetique:erreurs}.
\end{prob-solution}
