...
\begin{objectifs}
\item Comprendre l’effet de la représentation interne des nombres dans
  un ordinateur sur la précision des calculs.
\item Savoir convertir un nombre décimal vers une base quelconque, et
  vice versa.
\item Savoir exprimer un nombre décimal dans la représentation interne
  d’un ordinateur selon la norme IEEE~754.
\item Savoir planifier ses calculs avec un ordinateur de manière à
  diminuer l’impact des erreurs d’arrondis et de troncature et
...
\begin{gotoR}
  ...
  section~\ref{sec:ordinateurs:code} pour voir quelques exemples de
  calculs simples que R n'arrive apparemment pas à effectuer
  correctement. Tous les exemples sont tirés de messages envoyés à la
...
aussi fondamentale pou un langage mathématique que le calcul
...
foire aux questions de R\footnote{%
...
Les ordinateurs ne savent pas faire d'arithmétique à proprement
...
section~\ref{sec:ordinateurs:unites}, introduit les principales unités
...
section~\ref{sec:ordinateurs:arithmetique} explique comment ces
...
\begin{exemple}
  ...
  \end{displaymath}
  Ce nombre a les représentation suivantes dans d'autres bases. En binaire:
  \begin{align*}
    101011100_{2}
...
\begin{exemple}
  ...
  fractionnaire ($n = 5$). Le tableau~\ref{tab:ordinateurs:10_vers_b}
  montre le processus en suivant les étapes de
  l'algorithme~\ref{algo:ordinateurs:10_vers_b}. On obtient le
  résultat final en combinant la dernière colonne du
  tableau~\ref{tab:ordinateurs:10_vers_b:a} lue de bas en haut avec la
  dernière colonne du tableau~\ref{tab:ordinateurs:10_vers_b:b} lue de
  ...
  \begin{table}[tbp]
    ...
    \begin{minipage}[t]{0.45\linewidth}
      ...
      \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}crrcc}
        \toprule
        $i$ &
        \multicolumn{1}{c}{$v$} &
...
\begin{exemple}
  ...
  \item en déplaçant la virgule de cinq positions vers la gauche, on
    obtient $10111,01001$, comme à
    l'exemple~\ref{ex:ordinateurs:10_vers_b}.
  \end{enumerate}
  \qed
\end{exemple}
...
l'exemple~\ref{ex:ordinateurs:10_vers_b:bis} (la partie fractionnaire
...
l'algorithme~\ref{algo:ordinateurs:b_vers_10} et, finalement, de
diviser le nombre obtenu par $b^n$.

\begin{exemple}
  \label{ex:ordinateurs:b_vers_10}
  Soit le nombre binaire $101011,011$ que l'on convertit en décimal.
  Le tableau~\ref{tab:ordinateurs:b_vers_10} illustre le processus de
  conversion selon l'algorithme~\ref{algo:ordinateurs:b_vers_10} (le
  chiffre en surbrillance est celui traité lors de chaque étape de
  ...
  \begin{table}[tbp]
    \caption{Conversion d'un nombre binaire en décimal}
...
Il existe de nombreuses bases d'usage courant où chaque symbole est
(potentiellement) exprimé dans une base différente. On a qu'à penser à
l'heure ou l'ensemble du système de mesure impérial, par exemple. Or,
il peut s'avérer utile de savoir convertir de et vers une base
quelconque. Le matériel de cette section se retrouve dans très peu de
d'ouvrages d'analyse numérique ou de statistique numérique. Pourtant,
la conversion de et vers des bases générales recèle quelques
applications intéressantes, comme nous le verrons.
...
différente. On peut restreindre la discussion aux entiers sans perte
de généralité. On a donc
\begin{displaymath}
  x = x_{m-1}x_{m-2} \cdots x_1x_0,
\end{displaymath}
...
\begin{exemple}
  ...
  \begin{align*}
    ...
    &= 95,600 \text{ secondes}.
  \end{align*}
  À l'inverse, le nombre de jours, heures, minutes et secondes
  correspondant à \nombre{91492} secondes est, en utilisant
  l'algorithme \ref{algo:ordinateurs:10_vers_b} avec la base $[365\;
  24\; 60\; 60]$: 1 jour, 1 heure, 24 minutes et 52 secondes (voir le
  tableau~\ref{tab:ordinateurs:10_vers_generale} pour les calculs).
  \begin{table}
    \caption{Conversion d'un nombre décimal dans la base générale $[365\; 24\;
      60\; 60]$.}
...
\begin{exemple}
  ...
  \begin{align*}
   ...
   1, alors que les systèmes de numération commencent à 0.%
   \qed
\end{exemple}
...
pour de la plupart des langues occidentale requiert un octet; voir la
section~\ref{sec:ordinateurs:codage}. Les termes pour de plus grandes
quantités de bits ou d'octets utilisent généralement les préfixes
usuels du système international d'unités. Par exemple:
\begin{itemize}
\item 1 kilooctet (ko) est $2^{10} = 1,024$ octets;
\item 1 mégaoctet (Mo) est $2^{20} = 1,048,576$ octets;
\item 1 gigaoctet (Go) est $2^{30} = 1,073,741,824$ octets.
\end{itemize}
...
\item Les fabricants de disques durs divisent la capacité réelle de
  ...
  une capacité de $93,132$ gibioctets. Moins vendeur...
\end{enumerate}
...
réels dans les ordinateurs d'usage courant. Il est nécessaire de
...
ordinateur, bien que vaste de nos jours, demeure limitée. Par
conséquent, les nombres y sont représentés par un ensemble de $m$
bits. Habituellement, $m$ est un multiple de 2 et sa valeur détermine
le type de nombre. Par exemple, des définitions usuelles sont $m = 2^4
= 16$ pour un entier, $m = 2^5 = 32$ pour un nombre réel en simple
précision (\emph{float} dans plusieurs langages de programmation) et
$m = 2^6 = 64$ pour un nombre réel en double précision
(\emph{double}).
...
Nous décrivons plus en détail la norme pour les nombres en
\emph{double précision} puisque c'est le type avec lequel R travaille
toujours. Tout d'abord, un nombre en double précision est stocké dans
un mot de $m = 64$~bits (8~octets) et divisé ainsi: $m_S = 1$ bit pour
le signe, $m_E = 11$ bits pour l'exposant et $m_F = 52$ bits pour la
partie fractionnaire de la mantisse. Voir la
figure~\ref{fig:ordinateurs:double} pour une représentation
schématique.
...
tableau~\ref{tab:ordinateurs:special_values}. Par conséquent, les
...
\begin{exemple}
  ...
  section~\ref{sec:ordinateurs:conversion:10_vers_b}, le nombre
  ...
  l'algorithme~\ref{algo:ordinateurs:10_vers_b}, on trouve (le côté
...
\item Soit $x_{\text{min}}$ le plus petit nombre normalisé
  ...
  \end{align*}
  Afin de rendre la transition vers 0 moins abrupte, la norme IEEE~754
  définit également des nombres \emph{dénormalisés}. Ceux-ci sont
  identifiés par $E = 0$ et une partie fractionnaire $F$ non nulle.
  Cependant, par définition, les nombres dénormalisés ont $E - e =
  -\nombre{1022}$ et leur bit caché est $0$. Par conséquent, le plus
  petit nombre dénormalisé est stocké sous la forme
  \begin{equation*}
...
Le tableau~\ref{tab:arithmetic:simplified} fournit quelques exemples.
...
Lors de l'addition et la soustraction de nombres en virgule flottante,
on rend les exposants égaux en déplaçant les bits de la mantisse du
plus petit nombre vers la droite. Il en résulte une perte de bits
significatifs et donc une \emph{erreur d'arrondi}. Dans les cas
extrêmes où les deux opérandes sont de grandeur très différente, le
plus petit opérande devient nul suite à la perte de tous ses bits
significatifs.
...
\begin{enumerate}[label=(P\arabic*),font=\bfseries]
...
\begin{exemple}
  L'évaluation numériques de probabilités loin à droite dans la queue
  d'une fonction de répartition peut s'avérer imprécise selon la
  technique employée. Par exemple, la fonction de répartition de la
  distribution logistique est
  \begin{equation*}
    F(x) = (1 + e^{-x})^{-1}.
  \end{equation*}
...
\begin{enumerate}[label=(P\arabic*),font=\bfseries,start=5]
...
\begin{exemple}
  ...
  \begin{equation*}
    \hat{\alpha} = \frac{n}{%
      \ln \big( \frac{X_1 \cdots X_n}{X_{(1)}^n} \big)},
  \end{equation*}
...
@ %
  ...
  section~\ref{sec:ordinateurs:arithmetique:cout}), en plus d'ouvrir
  la porte à des erreurs d'arrondi. Pour obtenir une réponse on
  utilisera plutôt une autre formule algébriquement équivalente:
  \begin{displaymath}
...
indicatif, on trouve au tableau \ref{tab:ordinateurs:cout} le coût
...
de ligne, retour de chariot ou espacement arrière). Par exemple, les
lettres majuscules A--Z occupent les cases 65--90 (1000001--1011010 en
binaire) alors que les lettres minuscules a--z occupent les cases
97--122 (1100001--1101010 en binaire). Ainsi, on constate que les
versions majuscule et minuscule d'une même lettre ne diffèrent, dans
leur représentation binaire, que par leur second bit le plus
significatif. Le changement de casse est donc une opération très
simple et rapide.
