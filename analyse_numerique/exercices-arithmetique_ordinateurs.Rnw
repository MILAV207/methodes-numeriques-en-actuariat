...
\begin{exercice}
  ...
  \item Utiliser l'algorithme de conversion des nombres en base $b$
    ...
    l'exemple~\ref{ex:ordinateurs:position_dans_matrice} pour trouver
  ...
  \begin{sol}
    ...
    \item Dans l'ordre R, on convertit le nombre $[k - 1\; j - 1\; i -
      ...
      l'exercice~\ref{operateurs}.\ref{exercice:operateurs:ijk}~b) de
      la partie~I.
    \end{enumerate}
  \end{sol}
  \begin{rep}
...
\begin{exercice}
  ...
  \item C'est maintenant le temps d'expliquer le trait d'humour dans
    le strip de \textsf{XKCD.com} au début du document. Convertir en
    décimal les représentations en complément à deux des nombres sur
    8~bits de $\fbox{00000000}$ à $\fbox{11111111}$. Répéter
    l'exercice sur 16~bits. En fait, convertir non pas tous les
    nombres, mais bien seulement quelques valeurs clés qui permettront
    d'observer la séquence des nombres décimaux.
  ...
  \begin{sol}
    ...
    \item L'ensemble des nombres admissibles pour le type
      \texttt{Byte} est $\{0, \dots, 2^8 - 1\} = \{0, \dots, 255\}$.
      Pour le type \texttt{Integer}, on a de la partie c) que les
      nombres admissibles sont les entiers $\{-2^{15}, \dots, 2^{15} -
      1\} = \{-\nombre{32768}, \dots, \nombre{32767}\}$. Par analogie,
      on trouve que l'ensemble des entiers admissibles pour le type
      \texttt{Long} est $\{-2^{31}, \dots, 2^{31} - 1\} =
...
\begin{exercice}
  ...
  \begin{sol}
    Voir la section~\ref{sec:ordinateurs:ieee}. Les calculs sont
