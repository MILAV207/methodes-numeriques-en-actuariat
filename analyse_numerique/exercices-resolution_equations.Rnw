...
\begin{exercice}
  ...
  section~\ref{sec:resolution:code}, écrire une fonction R pour
...
\begin{exercice}
  Considérer la fonction $g(x) = 2^{-x}$ sur l'intervalle $[0,
  1]$.
  \begin{enumerate}
  \item Vérifier si les hypothèses du
    théorème~\ref{thm:resolution:pointfixe} quant à l'existence et
  ...
  \begin{sol}
    ...
    \item On considère la fonction $g(x) = 2^{-x}$ dans l'intervalle
      ...
      \begin{displaymath}
        ...
        0 \leq x \leq 1.
      \end{displaymath}
      Par le théorème~\ref{thm:resolution:pointfixe}, la fonction $g$
      possède un point fixe unique dans l'intervalle $[0, 1]$.
    \item Le graphique de la fonction $g$ se trouve à la figure
      \ref{fig:resolution:2^-x}. On constate que la fonction a effectivement un
...
\begin{exercice}
  ...
  l'exemple~\ref{ex:resolution:pointfixe_2} ont toutes un point fixe
...
\begin{exercice}
  Vérifier graphiquement que le théorème~\ref{thm:resolution:pointfixe}
  demeure valide si la condition $|g^\prime(x)| \leq k < 1$ est
  remplacée par $g^\prime(x) \leq k < 1$.
  \begin{sol}
    La condition $|g^\prime(x)| \leq k < 1$ du
    théorème~\ref{thm:resolution:pointfixe} assure l'unicité d'un
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \end{align*}
    La fonction \code{tri} de la figure~\ref{fig:resolution:tri} calcule le
...
\begin{exercice}
  Refaire l'exercice \ref{chap:resolution}.\ref{ex:resolution:tri} en
  VBA en composant une fonction accessible dans une feuille Excel et
  comparer le résultat avec la fonction Excel \code{TRI()}.
  \begin{sol}
...
\begin{exercice}
  ...
  \begin{enumerate}
  \item Écrire une fonction R pour estimer par la méthode du maximum
  ...
  \begin{sol}
    ...
    \item On veut estimer par la méthode du maximum de vraisemblance
      ...
      \end{displaymath}
      La figure \ref{fig:resolution:emv.gamma} présente une fonction R pour
...
\begin{exercice}
  ...
  l'exercice~\ref{avance}.\ref{exercice:avance:pareto} de la partie~I.
...
@
    ...
    (section~\ref{sec:resolution:astuce}) consistant à estimer le
