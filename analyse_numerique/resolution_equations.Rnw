...
\begin{exemple}
  ...
  $a_{\angl{10}\,i} = 8,2218$. On a déjà déterminé à l'\autoref{ex:resolution:bissection_2} que le taux d'intérêt se trouve
...
\begin{exemple}
  ...
  On reprend l'\autoref{ex:resolution:pointfixe_1} de calcul d'un
  ...
  plus efficace pour résoudre ce genre de problème. La \autoref{fig:resolution:newtonraphson_1} montre pourquoi elle est plus
...
@
  ...
  fonction \code{pointfixe} mentionnée à la \autoref{sec:resolution:pointfixe}, sauf qu'on y a ajouté un argument
...
\begin{exemple}
  ...
  \end{align*}
  Celle-ci est présentée à la \autoref{fig:resolution:newtonraphson_2}. On constate que la procédure
...
\begin{exemple}
  ...
  $x = 2$, ce qui cause quelques soucis. (Voir la \autoref{fig:resolution:newtonraphson_3-1} pour un graphique de cette
  fonction.)
  \begin{figure}[t]
    \centering
<<echo=FALSE, fig=TRUE>>=
...
@
  ...
  \begin{align*}
    ...
    &= 4x^2 - 14x + 14.
  \end{align*}
  Le graphique de la fonction $g(x)$ ci-dessus se trouve à la \autoref{fig:resolution:newtonraphson_3-2}. On notera toutefois que le
