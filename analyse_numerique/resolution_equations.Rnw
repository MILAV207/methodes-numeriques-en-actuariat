...
\begin{objectifs}
\item Être en mesure de résoudre une équation à une variable à l’aide
  des méthodes de la bissection, du point fixe et de Newton-Raphson.
\item Réaliser une mise en {\oe}uvre des méthodes de résolution
  ci-dessus en VBA et en R.
\item Savoir quand utiliser une méthode plutôt qu’une autre.
\item Pouvoir déterminer si une fonction admet un point fixe dans un
  intervalle donné.
\item Connaître et savoir utiliser les fonctions de résolution
  d’équations et d’optimisation de Excel et R.
\end{objectifs}
...
simple en apparence seulement, survient dans une foule de domaines.
Nous donnons deux exemples, tiré des mathématiques financières et de
...
cherché. L'algorithme~\ref{algo:resolution:bissection} systématise
cette idée et la figure~\ref{fig:resolution:bissection} l'illustre
...
abline(h = 0)
segments(x, -2.08, x, f(x), lty = c(1, 1, 2))
...
@
  \caption{Illustration de la méthode de bissection. À chaque
    ...
    sait se trouver la solution de l'équation $f(x) = 0$. Le point
...
L'application de l'algorithme~\ref{algo:resolution:bissection} génère
...
\begin{rems}
  ...
  \item À l'étape~\ref{algo:resolution:bissection:signe} de
    ...
    valeurs négatives ou deux valeurs positives) ou de part et d'autre
  ...
  \item Nous avons utilisé l'erreur relative comme critère d'arrêt à
    ...
    \ll x^*$ (voir la figure~\ref{fig:resolution:critere} pour deux
...
\begin{exemple}
  ...
  Le code R de la section~\ref{sec:resolution:code} définit une
  ...
  \item une valeur booléenne indiquant si les calculs intermédiaires
    doivent être affichés à l'écran: si l'argument est \code{TRUE},
    les valeurs de $a$, $b$, $x_n$ et $f(x_n)$ sont affichées à chaque
    itération.
  \end{enumerate}
  ...
  \begin{gotoR}
    ...
    section~\ref{sec:resolution:code} pour définir la fonction
...
\begin{exemple}
  ...
  La valeur présente d'une série de 10 paiements de fin d'année est
  ...
  \begin{gotoR}
    Exécuter le code informatique de la
    section~\ref{sec:resolution:code} correspondant à cet exemple pour
    la solution.
  \end{gotoR}
  \qed
\end{exemple}
...
fonctions $g(x)$ et $y = x$ sont égales, c'est-à-dire où la fonction
...
\begin{equation*}
  g(x) = x - f(x),
\end{equation*}
les problèmes de trouver $x$ tel que $g(x) = x$ ou $f(x) = 0$ sont
équivalents.
...
\begin{proof}
  ...
  \item Le premier volet garantit l'existence d'un point fixe sur $[a,
    ...
    figure~\ref{fig:resolution:demo}.
    ...
    Observons le graphique (a) de la figure~\ref{fig:resolution:demo}:
    \begin{itemize}
...
@
    ...
    \end{minipage}
    \caption{Exemples de graphiques permettant de faire la démonstration du théorème du point fixe}
...
section~\ref{sec:resolution:bissection}, toutes les méthodes
...
théorème~\ref{thm:resolution:pointfixe:2} nous dit que pour trouver un
point fixe, le meilleur essai $x_n$ à faire est la valeur de la
fonction $g$ à l'essai précédent, $g(x_{n - 1})$. La
figure~\ref{fig:resolution:pointfixe} illustre comment la suite des
essais succesifs peut converger vers le point fixe.
...
segments(x[1], 6.95, x[1], x[2], lty = 2)
segments(x[-1], x[-1], x[-1], g(x[-1]), lty = 2)
...
arrows(head(x, -1), x[-1], x[-1], x[-1],
       col = "blue", length = 0.1)
@
  \caption{Illustration de la méthode du point fixe pour une fonction
    décroissante et une fonction croissante. Les points rouges
...
\begin{rem}
  ...
  figure~\ref{fig:resolution:pointfixe}, la rapidité de la convergence
...
\begin{exemple}
  ...
  On répète l'exemple~\ref{ex:resolution:bissection_2} à l'aide de la
  méthode du point fixe, soit trouver le taux d'intérêt $i$ tel que
  $a_{\angl{10}\,i} = 8,2218$. On a déjà déterminé à l'exemple
  \ref{ex:resolution:bissection_2} que le taux d'intérêt se trouve
  dans l'intervalle $[0,035, 0,040]$. On peut exprimer le problème
  sous forme de point fixe ainsi: trouver la valeur de $i$ tel que
  \begin{displaymath}
  ...
  présentée à la section \ref{sec:resolution:bissection}. Ses
  arguments sont d'ailleurs les mêmes, à l'exception que les points
  $a$ et $b$ qui sont remplacés par une valeur de départ $x_0$.

  \begin{gotoR}
    Exécuter le code informatique de la
    section~\ref{sec:resolution:code} correspondant à ce bloc de
...
res <- pointfixe(g, start = 0.0375)
@
  Il faut \Sexpr{res$nb.iter} %$
...
\begin{exemple}
  ...
  On répète l'exemple \ref{ex:resolution:bissection_1} à l'aide de la
  ...
  Les graphiques des cinq fonctions ci-dessus se trouvent aux figures
  \ref{fig:resolution:pointfixe_2-1}--\ref{fig:resolution:pointfixe_2-5}.
  L'examen de ces graphiques permet de déterminer, avant de faire tout
  calcul, pour quelles fonctions la procédure itérative du point fixe
  convergera ainsi que les taux de convergence relatif, le cas
  échéant.
  \begin{figure}
...
@
      \caption{Fonction $g_1(x)$}
      \label{fig:resolution:pointfixe_2-1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\linewidth}
      \centering
...
polygon(c(1, 1, 2, 2), c(1, 2, 2, 1),
        lty = "dashed", border = "blue")
@
      \caption{Fonction $g_5(x)$}
      \label{fig:resolution:pointfixe_2-5}
    \end{minipage}
  \end{figure}

  \begin{gotoR}
    Compléter cet exemple en exécutant le code informatique
    correspondant à la section~\ref{sec:resolution:code}.
  \end{gotoR}
  \qed
\end{exemple}
...
L'étude de la méthode de Newton-Raphso nous ramène au problème de
trouver la racine $x^*$ de la fonction $f(x)$, c'est-à-dire la
solution de
\begin{displaymath}
  f(x) = 0.
\end{displaymath}
...
tangente <- function(x) fp(0.7) * x + f(0.7) - 0.7 * fp(0.7)
curve(f(x), xlim = c(-0.8, 1.2), ylim = c(-1, 2), lwd = 2)
abline(h = 0)
xh <- -(f(0.7) - 0.7 * fp(0.7))/fp(0.7)
points(c(0.2, 0.7, xh), c(0, f(0.7), f(xh)),
       pch = c(19, 21, 21), bg = "red3")
segments(c(0.7, xh), c(-1.1, 0), c(0.7, xh), f(c(0.7, xh)),
         lty = 2)
arrows(1.1, tangente(1.1), xh, 0, col = "blue", length = 0.1)
@
  \caption{}
  \label{fig:resolution:nr}
\end{figure}
...
\begin{exemple}
  ...
  On répète l'exemple \ref{ex:resolution:pointfixe_1} à l'aide de la
  méthode de Newton--Raphson.  On sait du cours de mathématiques
  financières que la méthode de Newton--Raphson est la plus efficace
  pour résoudre les problèmes de recherche d'un taux de rendement. La
  figure \ref{fig:resolution:newtonraphson_1} montre pourquoi elle est
  plus efficace que la méthode du point fixe: la fonction
  \begin{displaymath}
  ...
  \begin{displaymath}
    g(i) = \frac{1 - v^n}{k}
  \end{displaymath}
  utilisée à l'exemple \ref{ex:resolution:pointfixe_1}.
  \begin{figure}[t]
    \centering
<<echo=FALSE, fig=TRUE>>=
f <- function(x) x
...
@
  ...
  On a défini en R une fonction \code{nr} similaire à la
  fonction \code{pointfixe} mentionnée à la section
  \ref{sec:resolution:pointfixe}, sauf qu'on y a ajouté un argument
  pour la dérivée de la fonction $f(x)$.

  Le résultat de l'application de la fonction \code{nr} est
  le suivant:
<<echo=TRUE>>=
...
res <- nr(f, fp, 0.0375)
res.pf <- pointfixe(g, 0.0375)
@
  La méthode de Newton--Raphson ne requiert donc que \Sexpr{res$nb.iter}
  itérations pour obtenir un résultat équivalent à celui obtenu par la
  méthode du point fixe en \Sexpr{res.pf$nb.iter} itérations.

  À noter que, si la fonction $g$ est définie adéquatement, on peut de
  manière tout aussi équivalente utiliser la fonction
  \code{pointfixe} pour effectuer les itérations de la méthode de
  Newton--Raphson:
<<echo=TRUE>>=
...
\begin{exemple}
  ...
  On répète l'exemple \ref{ex:resolution:bissection_1} à l'aide de la
  méthode de Newton-Raphson. On a $f(x) = x^3 + 4 x^2 - 10$ et
  $f^\prime(x) = 3x^2 + 8 x$, d'où $f^\prime(x) \neq 0$ pour tout $x
  \in [1, 2]$. La fonction
  \begin{displaymath}
    g(x) = x - \frac{f(x)}{f^\prime(x)}
  \end{displaymath}
  est présentée à la figure \ref{fig:resolution:newtonraphson_2}, où
  l'on constate que la procédure itérative de Newton--Raphson
  convergera rapidement vers une racine unique dans l'intervalle $[1,
  2]$. En effet,
  \begin{figure}[t]
    \centering
<<echo=FALSE, fig=TRUE>>=
...
polygon(c(1, 1, 2, 2), c(1, 2, 2, 1),
        lty = "dashed", border = "blue")
@
    \caption{Fonction $g(x) = x - f(x)/f^\prime(x)$, où $f(x) = x^3 + 4 x^2 -
      10$}
...
\begin{exemple}
  ...
  = (4x - 7)/(x - 2)$ a une racine en $x = 1,75$. une asymptote en $x
  = 2$, ce qui cause quelques soucis. (Voir la figure
  \ref{fig:resolution:newtonraphson_3-1} pour un graphique de cette
  fonction.)
  \begin{figure}[t]
    \centering
<<echo=FALSE, fig=TRUE>>=
...
abline(8, -4, lty = "dotted", lwd = 2, col="gray")
abline(h = 0)
abline(v = 2, lty = "dashed", col="blue")
@
  \caption{Fonction $f(x) = (4x - 7)/(x - 2)$ (ligne épaisse), asymptote
    (ligne traitillée) et tangente en $x = 1,5$ (ligne pointillée)}
  ...
  \begin{align*}
    ...
    &= 4x^2 - 14x + 14.
  \end{align*}
  Le graphique la fonction $g(x)$ se trouve à la figure
  \ref{fig:resolution:newtonraphson_3-2}. On notera toutefois que le
  point fixe en $x = 2$ n'est pas une solution acceptable dans le
  problème d'origine.
\begin{figure}[t]
    \centering
<<echo=FALSE, fig=TRUE>>=
...
lim <- c(1.5, 3)
curve(g, xlim = lim, ylim = lim, ylab = "g(x)", lwd = 2)
curve(f0, add = TRUE)
@
  ...
  Avec une valeur de départ $x_0 = 1,625 < 1,75$, la convergence se
  fait vers la bonne racine:
<<echo=TRUE>>=
f <- function(x) ifelse(x == 2, NA, (4*x - 7)/(x - 2))
...
@
  ...
  On peut vérifier à la figure
  \ref{fig:resolution:newtonraphson_3-2} qu'avec une valeur de départ
  entre $1,75$ et $2$, la procédure itérative convergera vers la bonne
  réponse:
<<echo=TRUE>>=
pointfixe(g, 1.875, echo=TRUE)
@

  La tangente en $x = 1,5$ tracée sur le graphique de la figure
  \ref{fig:resolution:newtonraphson_3-1} montre que $x_0 = 1,5$
  constitue un mauvais choix car $g(1,5) = 2$. La procédure
  itérative converge donc vers une  valeur non admissible:
<<echo=TRUE>>=
pointfixe(g, 1.5)
@

  Pour toute valeur de départ supérieure à $2$, la procédure
  itérative diverge. On peut vérifier ce fait graphiquement autant à
  la figure \ref{fig:resolution:newtonraphson_3-1} qu'à la figure
  \ref{fig:resolution:newtonraphson_3-2}:
\begin{Schunk}
\begin{Sinput}
...
[1] 7.34684e+88
[1] 2.159042e+178
Erreur dans if (abs(x - xt)/abs(x) < TOL) break :
...
ces méthodes pour les systèmes à plusieurs variables comme
\begin{align*}
  f_1(x_1, x_2, x_3) &= 0 \\
  f_2(x_1, x_2, x_3) &= 0 \\
  f_3(x_1, x_2, x_3) &= 0.
...
disponibles dans Excel, S-Plus et R.
...
exemples dans le classeur
\url{...\Microsoft Office\Office11\Samples\SOLVAMP.XLS}.


\subsubsection{R}

Consulter le chapitre 7 de \cite{Goulet:introS:2007} pour une
description des fonctions d'optimisation disponibles dans R.
