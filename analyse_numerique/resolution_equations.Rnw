...
\begin{prob-enonce}
  Les fonctions décrivant les heures de lever et de coucher du
  soleil à Québec pour le mois de janvier 2016 sont
  représentées par $L(x)$ et $C(x)$, respectivement :
  
  \begin{align*}
    L(x) = \mathrm{e}^{-0.001431x+2.016340}\\
    C(x) = \mathrm{e}^{0.001312x+2.778538}\\
  \end{align*}
  
  , où x est le jour du mois (un entier).
  
  On définit la fonction du nombre d'heures d'ensoleillement
  pour une journée donnée par $H(x) = C(x) - L(x)$.
  
  Pendant quel jour du mois les Québécois bénéficient-ils
  d'exactement 9h d'ensoleillement, à une minute près?
\end{prob-enonce}
...
\begin{prob-astuce}
  Afin de déterminer pour quelle valeur de $x$ la fonction
  $H(x)$ vaut 9, on devra donc trouver la solution à l'équation
  $H(x) - 9 = 0$, ou $H(x) = 9$.
  
  On pourrait y aller avec une technique d'essai-erreur
  \emph{intuitive}, c'est à dire :
  
  \begin{enumerate}
    \item Évaluer le nombre d'heures d'ensoleillement aux bornes
      de l'intervalle afin de s'assurer que la valeur 9 se trouve
      dans l'intervalle, puis déterminer si la fonction est croissante
      ou décroissante sur l'intervalle.
      
      Note : La valeur 9 est comprise dans l'intervalle et la fonction
      d'heures d'ensoleillement est croissante.
    \item Faire une prédiction initiale dans l'intervalle
      $(1, 31)$, et calculer le nombre d'heures d'ensoleillement.
    \item Si le nombre d'heures précédent est trop grand,
      faire une nouvelle prédiction inférieure à la précédente. Si
      le nombre d'heures précédent est trop petit,
      faire une prédiction supérieure à la précédente.
    \item Répéter l'étape 2 jusqu'à temps de converger vers le
      bon jour à un degré de précision désiré.
  \end{enumerate}
  
  En procédant ainsi, nous obtiendrions les résultats suivants :
...
# Par les valeurs aux bornes, nous notons que
# la fonction est croissante et que
# la valeur 9 est comprise dans l'intervalle

# Le jour semble à la mi-mois.
# Essayons j1 = 15

h(15)

# La valeur est supérieure à 9.
# Le jour recherché se trouve donc entre
# 1 et 15.
# Essayons j2 = 10

h(10)

# La valeur est inférieure à 9.
# Le jour recherché se trouve donc entre
# 10 et 15.
# Essayons j3 = 13
...
\begin{prob-astuce}
  La méthode de bissection est en fait une automatisation de
  la technique d'essai-erreur \emph{intuitive} présentée à
  l'astuce précédente.
  
  Selon l'\autoref{algo:resolution:bissection}, on doit d'abord
  poser l'équation à résoudre sous la forme
  $f(x^*) = 0$, où $x^*$ est le jour recherché :
  
  \begin{align*}
    &\mathrm{e}^{0.001312x^*+2.778538}
      - \mathrm{e}^{-0.001431x^*+2.016340}
      = 9\\
    &H(x^*) = 9\\
    &H(x^*) - 9 = 0\\
    &\Rightarrow \quad
  ...
  Puis, on applique l'algorithme de bissection avec
  $f(x^*) = H(x^*) - 9$, $a=1$ et $b=31$. Pour une mise en oeuvre de
  l'algorithme en R, voir la \autoref{chap:resolution:prob-solution}.
\end{prob-astuce}
...
\begin{prob-astuce}
  Selon l'\autoref{algo:resolution:pointfixe}, la  méthode du point-fixe
  recquiert d'isoler x de façon à obtenir
  une équation sous la forme $ x^* = g(x^*) $. On reprend donc l'équation
  de départ, et on isole $x^*$ :
  
  \begin{align*}
    \mathrm{e}^{0.001312x^*+2.778538} - \mathrm{e}^{-0.001431x^*+2.016340}
      &= 9\\
    \mathrm{e}^{0.001312x^*+2.778538}
      &= 9 + \mathrm{e}^{-0.001431x^*+2.016340}\\
    0.001312x^*+2.778538
      &= \ln\left(9 + \mathrm{e}^{-0.001431x^*+2.016340}\right)\\
    x^*
      &= \frac{\ln\left(9 + \mathrm{e}^{-0.001431x^*+2.016340}\right)
        - 2.778538}{0.001312}\\
    \Rightarrow g_1(x^*) 
      &= \frac{\ln\left(9 + \mathrm{e}^{-0.001431x^*+2.016340}\right)
        - 2.778538}{0.001312}\\
  ...
  Comme mentionné plus tôt, une deuxième fonction $g_2(x^*)$ aurait pu être
  obtenue en isolant l'autre $x^*$ dans l'équation de départ.

  Il ne reste plus qu'à appliquer l'algorithme du point fixe avec $g_1(x^*)$
  ou $g_2(x^*)$ et une valeur de départ $x_0 \in (1, 31)$ au choix.
  Pour une mise en oeuvre de l'algorithme en R, voir la
  \autoref{chap:resolution:prob-solution}.
\end{prob-astuce}
...
\begin{prob-astuce}
  La méthode de Newton-Raphson s'applique bien puisque la fonction
  des heures d'ensoleillement est facile à dériver. Selon
  l'\autoref{algo:resolution:newtonraphson}, on doit d'abord
  poser l'équation à résoudre sous la forme $f(x^*) = 0$. On reprend
  donc le résultat de la méthode de bissection :
  
  \begin{align*}
    f(x^*)
    &= H(x^*) - 9\\
    &= \mathrm{e}^{0.001312x^*+2.778538}
      - \mathrm{e}^{-0.001431x^*+2.016340} - 9\\
  \end{align*}
  
  Afin de mettre en oeuvre l'algorithme, on doit ensuite dériver
  la fonction $f(x)$ par rapport à $x$ :
  
  \begin{align*}
    f'(x)
    &= \frac{df(x)}{dx}\\
    &= 0.001312 \mathrm{e}^{0.001312x+2.778538}
      + 0.001431 \mathrm{e}^{-0.001431x+2.016340}\\
  \end{align*}

  Il ne reste qu'à appliquer la méthode de Newton-Raphson avec les fonctions
  $f(x)$ et $f'(x)$, et une valeur de départ $x_0 \in (1, 31)$ au choix.
  Pour une mise en oeuvre de l'algorithme en R, voir la
  \autoref{chap:resolution:prob-solution}.
\end{prob-astuce}
...
\begin{prob-astuce}
  ...
  à la \autoref{chap:resolution:prob-solution} pour confirmer les autres
  résultats.
\end{prob-astuce}
...
\begin{prob-solution}
  \label{chap:resolution:prob-solution}
  Note : les fonctions \code{bissection}, \code{pointfixe}
  et \code{nr} sont celles tirées de la \autoref{sec:resolution:code}.
...
bissection <- function(FUN, lower, upper, TOL = 1E-6, MAX.ITER = 100, echo = FALSE)
{
    ...
    if (FUN(lower) * FUN(upper) > 0)
        stop('FUN(lower) and FUN(upper) must
...
bissection(FUN = function(x) h(x) - 9, lower = 1,
           upper = 31, TOL = 1E-6, MAX.ITER = 100,
           echo = FALSE)
@
...
pointfixe(FUN = g, start = 15, TOL = 1E-6,
...
nr(FUN = function(x) h(x) - 9, FUNp = fp, start = 15,
   TOL = 1E-6, MAX.ITER = 100, echo = FALSE)
@


  Fonction \code{uniroot} :

<<echo=TRUE>>=
uniroot(function(x) h(x) - 9, c(1, 31))
@
