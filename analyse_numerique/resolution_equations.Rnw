...
\section[Méthode de Newton--Raphson]{%
  Méthode de Newton-Raphson}         % MyriadPro n'aime pas --
...
figure~\ref{fig:resolution:newtonraphson} pour une illustration.
...
arrows(1.2, tangente(1.2, x[1]), x[2], 0, col = "blue", length = 0.07)
arrows(0.95, tangente(0.95, x[2]), x[3], 0, col = "blue", length = 0.07)
...
figure~\ref{fig:resolution:newtonraphson}, la tangente de $f$ en
...
Tel qu'illustré à la figure~\ref{fig:resolution:newtonraphson}, on
...
L'algorithme~\ref{algo:resolution:newtonraphson} systématise cette
procédure.
...
\begin{exemple}
  ...
  On reprend l'exemple~\ref{ex:resolution:pointfixe_1} de calcul d'un
  taux de rendement, cette fois à l'aide de la méthode de
  Newton--Raphson. Vous savez peut-être de votre cours de
  mathématiques financières que la méthode de Newton--Raphson est la
  plus efficace pour résoudre ce genre de problème. La figure
  \ref{fig:resolution:newtonraphson_1} montre pourquoi elle est plus
  efficace que la méthode du point fixe: la fonction
  \begin{equation*}
...
@
  ...
  \begin{gotoR}
    Exécuter le code informatique de la
    section~\ref{sec:resolution:code} correspondant à ce bloc de
    matière pour définir la fonction \code{nr} puis résoudre cet
    exemple. Prendre note comment l'on peut également utiliser la
    fonction \code{pointfixe} pour obtenir le taux de rendement.
  \end{gotoR}
...
@
  ...
  l'exemple~\ref{ex:resolution:pointfixe_1} en
...
\begin{exemple}
  ...
  Revisitons l'exemple \ref{ex:resolution:bissection_1} où l'on
  ...
  \end{align*}
  Celle-ci est présentée à la figure
  \ref{fig:resolution:newtonraphson_2}. On constate que la procédure
  itérative de Newton--Raphson convergera rapidement vers une racine
  unique dans l'intervalle $[1, 2]$. En effet:
<<echo=TRUE>>=
f <- function(x) x^3 + 4*x^ 2 - 10
...
@
  ...
  l'exemple~\ref{ex:resolution:pointfixe_2}, celle pour laquelle la
  ...
  suivant l'algorithme~\ref{algo:resolution:newtonraphson}, à la
  page~\pageref{algo:resolution:rem}. %
  \qed
\end{exemple}
...
\begin{exemple}
  ...
  $x = 2$, ce qui cause quelques soucis. (Voir la figure
...
@
  ...
  \begin{align*}
    ...
    &= 4x^2 - 14x + 14.
  \end{align*}
  Le graphique de la fonction $g(x)$ ci-dessus se trouve à la figure
...
où celle-ci s'avère difficile et/ou inefficace à calculer, on peut
...
précédents; voir la figure~\ref{fig:resolution:secante}. Les valeurs
...
for (i in 1:(length(x) - 2))
{
    ...
    arrows(xx[2], fxx[2], xx[3], 0, col = "blue", length = 0.07)
}
...
\begin{gotoR}
  ...
  dans le code informatique de la section~\ref{sec:resolution:code}.
\end{gotoR}
...
J'ai appris le truc suivant dans un message de Brian Ripley --- un
important développeur de R --- posté dans les forums de discussion de
...
section~\ref{sec:resolution:code}.
