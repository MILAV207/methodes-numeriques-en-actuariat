...
\Opensolutionfile{solutions}[solutions-generateurs]
...
L'exercice \ref{rgl} requiert d'installer le package \textsf{R}
\texttt{rgl}, disponible sur CRAN%
...
\texttt{lib} de la fonction \texttt{install.packages}. Il faudra
également utiliser l'argument \texttt{lib.loc} de la fonction
\texttt{library} lors du chargement du package. Par exemple:
<<echo=TRUE, eval=FALSE>>=
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    fonction \texttt{rand} de la figure \ref{fig:rand}.
    \begin{figure}
      \centering
      \begin{framed}
\begin{verbatim}
...
\end{verbatim}
      \end{framed}
      \caption{Code de la fonction \texttt{rand}}
      \label{fig:rand}
    \end{figure}
<<echo=FALSE>>=
rand <- function(n, a, c, m, seed)
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
    \item On utilise de nouveau la fonction \texttt{rand} de
      la figure \ref{fig:rand}. Le graphique de la figure
      \ref{fig:a=17} a été créé avec les commandes
<<echo=TRUE, eval=FALSE>>=
...
plot(x[-length(x)], x[-1],
     xlab = expression(x[i]), ylab = expression(x[i+1]), pch = 19)
@
        \caption{Graphique de paires de valeurs du générateur congruentiel
...
@
      ...
      points sont plus uniformément disposés sur le graphique.
      \begin{figure}[btp]
        \centering
<<echo=FALSE,fig=TRUE>>=
x <- rand(n = 500, a = 85, c = 0, m = 2^13-1, seed = 19)
