...
\begin{osx}
  L'utilisation de \code{rgl} sous Mac OS~X, requiert l'application
  \code{X11.app}, qui n'est plus livrée avec le système depuis la
  version 10.8 (Mountain Lion). Un serveur et des bibliothèques client
  X11 compatibles avec les versions de OS~X 10.6 et suivantes sont
...
\begin{exercice}
  \begin{enumerate}
  \item Composer une fonction R faisant la mise en {\oe}uvre du générateur
