...
\begin{exercice}
  ...
  entière d'autant de nombres aléatoires provenant d'une loi
  exponentielle.
  \begin{enumerate}
  \item Démontrer que la partie entière d'une variable aléatoire
    exponentielle est distribuée selon une loi géométrique.
  \item Déterminer le paramètre $p$ de la loi géométrique en
    fonction du paramètre $\lambda$ de la loi exponentielle.
  \item Déterminer le paramètre $\lambda$ de la loi exponentielle
    en fonction du paramètre $p$ de la loi géométrique.
  \end{enumerate}
  \begin{rep}
    \begin{inparaenum}
      \stepcounter{enumi}
    \item Voir solution
    \item $p = 1-\mathrm{e}^{-\lambda}$
    \item $\lambda = -\ln(1-p)$
    \end{inparaenum}
  \end{rep}
  \begin{sol}
    \begin{enumerate}
    \item On définit la variable $X \tild Exponentielle(\lambda)$.
      
      On définit également $K = \lfloor X \rfloor$.

      En construisant la fonction de masse de probabilité de la
      variable aléatoire K, on remarque que celle-ci correspond
      en fait à celle d'une loi géométrique :

      \begin{align*}
        \Pr [K=0] &= \Pr[\lfloor X \rfloor = 0] =
                      \Pr[0 \leq X < 1]\\
                  &= F_X(1)\\
                  &= 1-\mathrm{e}^{-\lambda}\\
      \end{align*}
      \begin{align*}
        \Pr [K=1] &= \Pr[\lfloor X \rfloor = 1] = \Pr[1 \leq X < 2]\\
                  &= F_X(2) - F_X(1)\\
                  &= (1-\mathrm{e}^{-2\lambda}) - (1-\mathrm{e}^{-\lambda}) \\
                  &= (\mathrm{e}^{-\lambda}-\mathrm{e}^{-2\lambda})\\
                  &= (1-\mathrm{e}^{-\lambda})\mathrm{e}^{-\lambda}\\
      \end{align*}
      \begin{align*}
        \Pr [K=2] &= \Pr[\lfloor X \rfloor = 2] = \Pr[2 \leq X < 3]\\
                  &= \dots \\
                  &= (1-\mathrm{e}^{-\lambda})(\mathrm{e}^{-\lambda})^2\\
      \end{align*}
      \begin{align*}
        \Pr [K=k] &= \Pr[\lfloor X \rfloor = k] = \Pr[k \leq X < k+1]\\
                  &= (1-\mathrm{e}^{-\lambda})(\mathrm{e}^{-\lambda})^k,
                    \qquad \forall k \in (0, 1, 2, \dots).\\
      \end{align*}
    \item $\Pr [K=k] = p(1-p)^k, \qquad \forall k \in (0, 1, 2, \dots)$,
      \qquad où $ p = 1-\mathrm{e}^{-\lambda} $.
    \item $ p = 1-\mathrm{e}^{-\lambda} \qquad
      \Leftrightarrow \qquad \lambda = -\ln(1-p) $
    \end{enumerate}
  \end{sol}
\end{exercice}
