...
\begin{prob-enonce}
  L'intégrale de Gauss intervient dans plusieurs disciplines
  des mathématiques. En probabilités, elle est notamment à
  la base de la définition de la loi normale, ou loi
  gaussienne. En calcul intégral, elle est également un
  exemple fréquent de la résolution d'intégrales par la
  transformation du domaine en coordonnées polaires.
  
  À l'aide de la simulation, proposez une méthode
  alternative afin de résoudre numériquement l'intégrale
  
  \begin{equation*}
    G = \int_{-\infty}^{\infty} \mathrm{e} ^ {-x^2} \, \mathrm{d}x \\
  \end{equation*}
  
  , dont le résultat est connu et vaut $\sqrt{\pi}$.
\end{prob-enonce}
...
\begin{prob-astuce}
  Le domaine de l'intégrale est infini. Plusieurs changements
  de variable pourraient rendre le domaine à intégrer fini.
  En voici un exemple :

  \begin{align*}
    G &= \int_{-\infty}^{\infty} \mathrm{e} ^ {-x^2} \, \mathrm{d}x \\
      &= 2 \int_{0}^{\infty} \mathrm{e} ^ {-x^2} \, \mathrm{d}x
        \text{\qquad , par symétrie} \\
      &= 2 \left( \int_{0}^{1} \mathrm{e} ^ {-y^2} \, \mathrm{d}y
        + \int_{1}^{\infty} \mathrm{e} ^ {-x^2} \, \mathrm{d}x \right)
  \end{align*}

  L'intégrale de droite ayant un domaine infini, on pose
  une transformation sur $x$ afin de fournir un domaine
  fini à la procédure Monte Carlo :

  \begin{align*}
    u = x^{-1} \qquad
      &\Rightarrow \qquad \mathrm{d}u = - x^{-2} \, \mathrm{d}x \\
      &\Rightarrow \qquad \mathrm{d}x = -x^2 \, \mathrm{d}u
            = -u^{-2} \mathrm{d}u
  \end{align*}
  
  On note également que les bornes du domaine initial subissent
  la transformation $T(x_1,x_2)=(x_1^{-1}, x_2^{-1})$. Les bornes du nouveau
  domaine sont donc $T(1,\infty)=(1, 0)$. Puis on poursuit :
  
  \begin{align*}
    G &= 2 \left( \int_{0}^{1} \mathrm{e} ^ {-y^2} \, \mathrm{d}y
        + \int_{1}^{0} \mathrm{e} ^ {-u^{-2}} (-u^{-2} \, \mathrm{d}u)
          \right)\\
      &= 2 \left( \int_{0}^{1} \mathrm{e} ^ {-y^2} \, \mathrm{d}y
        + \int_{0}^{1} \mathrm{e} ^ {-u^{-2}} u^{-2} \, \mathrm{d}u
          \right)\\
      &= 2 \int_{0}^{1} \left( \mathrm{e} ^ {-y^2}
        + \mathrm{e} ^ {-y^{-2}} y^{-2} \right) \, \mathrm{d}y\\
  \end{align*}
  
  Cette intégrale est maintenant prête à être estimée
  à l'aide de la méthode Monte Carlo.
\end{prob-astuce}
...
\begin{prob-solution}
  Tel que démontré à l'astuce, le résultat de
  l'intégrale $G$ peut être obtenu en estimant
  le résultat de l'intégrale équivalente suivante :
  
  \begin{align*}
    \theta  &= 2 \int_{0}^{1} \left( \mathrm{e} ^ {-y^2}
              + \mathrm{e} ^ {-y^{-2}} y^{-2} \right) \, \mathrm{d}y \\
            &= 2 (1-0) \int_{0}^{1} \left( \mathrm{e} ^ {-y^2}
              + \mathrm{e} ^ {-y^{-2}} y^{-2} \right) \frac{1}{1-0} \, \mathrm{d}y \\
            &= 2 \Esp{\mathrm{e} ^ {-Y^2} + \mathrm{e} ^ {-Y^{-2}} Y^{-2}} \\
                    \text{, où } Y \sim U(0,1).\\
  \end{align*}

  Par conséquent, l'estimation est
  
  \begin{equation*}
    \hat{\theta}_n = \frac{2}{n} \sum_{i=1}^{n} {\mathrm{e} ^ {-y_i^2} + \mathrm{e} ^           {-y_i^{-2}} y_i^{-2}} \\
  \end{equation*}
  
  , où les $y_i$ sont tirés de la distribution $Y\sim U(0,1)$.
