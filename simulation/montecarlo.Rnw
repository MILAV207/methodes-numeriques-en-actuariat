...
plot(x, f(x), main = "n = 10 000",
     pch = 21, bg = "lightblue")
@
    \caption{Fonction $h(x)$ de l'exemple \ref{ex:montecarlo:2d} et points
      choisis aléatoirement où la fonction est évaluée pour l'intégration
      Monte Carlo}
...
@
    \caption{Fonction $h(x)$ de l'exemple
      \ref{ex:montecarlo:3d} et points choisis
