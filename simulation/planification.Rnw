...
\chapter[Planification d'une simulation en R]{%
  Planification d'une simulation \\ en R}
...
Pour ce faire, on doit d'abord simuler un grand nombre $N$
d'échantillons aléatoires de taille $n$ d'une distribution $U(\theta -
\frac{1}{2}, \theta + \frac{1}{2})$ pour une valeur de $\theta$
choisie. Pour chaque échantillon, on calculera ensuite les trois
...
alors on pourra conclure que $\hat{\theta}_i$ est sans biais. De même,
on déterminera lequel des trois estimateurs a la plus faible variance
...
\citep[chapitre~6]{Goulet:programmer-avec-r:2017}. On rappelle que la
syntaxe de ces fonctions est
\begin{quote}
  \code{lapply(x, FUN, ...)} \\
  \code{sapply(x, FUN, ...)}
\end{quote}
Ces fonctions appliquent la fonction \code{FUN} à tous les éléments de
...
passer à \code{sapply} pour obtenir les résultats de $N$
simulations. La \autoref{fig:planification:lapply} présente une
première version d'une telle fonction. On remarquera que l'argument
\code{i} ne joue aucun rôle dans la fonction. Voici un exemple
d'utilisation pour un petit nombre de simulations (4):
...
approche puisque l'on peut rapidement circonscrire un éventuel
problème à \code{fun1} ou \code{simul2}.


\section{Variante de la seconde approche}
\label{planification:replicate}
...
À des fins d'illustration, supposons que l'on utilise l'approche de la
...
\code{simul3.R}, dans l'ordre. Si l'on crée un autre fichier, disons
...
\begin{quote}
  \code{source("fun2.R")} \\
  \code{source("simul3.R")} \\
  \code{simul3(10000, 100, 0)}
\end{quote}
Une simple commande
...
auquel cas il faut spécifier le chemin d'accès complet de l'exécutable
à la ligne de commande:
\begin{quote}
\verb|"c:\Program Files\R\R-x.y.z\bin\R" CMD BATCH go.R|
\end{quote}
Remplacer \code{R-x.y.z} par le numéro de version courant de R.



\section{Conclusion}
\label{planification:conclusion}
