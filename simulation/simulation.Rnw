...
\begin{prob-enonce}
  ...
  région $R$ est un disque de rayon $1$, la probabilité que
  l'enveloppe convexe forme un triangle est
...
\begin{prob-astuce}
  Au c{\oe}ur du problème que nous tentons de résoudre, il y a la
  ...
  bidimensionnelle uniforme sur un disque de rayon $1$, dont la
  fonction de densité conjointe est
  \begin{equation*}
    f_{XY}(x, y) = \frac{1}{\pi}, \quad x^2 + y^2 < 1.
  \end{equation*}
...
x <- c(0.22, 0.58, 0.83)
eps <- eps <- 30E-4
segments(x - eps, 0, x - eps, dbeta(x, 3, 2), col = "darkgray", lwd = 3, lty = 2)
segments(x + eps, 0, x + eps, m, col = "darkgray", lwd = 3)
curve(dbeta(x, 3, 2), xlim = c(0, 1), add = TRUE, lwd = 2, col = "blue3")
segments(0, m, 1, m, col = "red3", lwd = 2)
...
\begin{exemple}
  ...
  \begin{figure}
    \centering
<<echo=FALSE, fig=TRUE>>=
curve(dbeta(x, 3, 2), xlim = c(0, 1), ylim = c(0, 2.5), col = "blue3", lwd = 2)
lines(c(0, 0.8, 1), c(0, 2.4, 0), lwd = 2, col = "red3")
segments(0.5, 0, 0.5, 1.5, col = "darkgray", lty = 2)
segments(0.8, 0, 0.8, 2.4, col = "darkgray", lty = 2)
axis(side = 1, at = 0.5)
@
...
plot(x[w], y[w], xlab="x", ylab="y", pch = 21, bg = "darkgray",
        xlim=c(-1, 1), ylim=c(-1, 1))
points(x[!w], y[!w], xlab="x", ylab="y", pch = 1)
lines(x = cos(t), y = sin(t), col = "blue3", lwd = 2)
rect(-1, -1, 1, 1, border = "red3", lwd = 2)
@
...
\begin{prob-astuce}
  La méthode acceptation-rejet présentée ci-haut peut être adaptée à
  notre problème en faisant fi des considérations probabilistes pour
  ne retenir que des arguments géométriques.

  En effet, une manière simple de générer des points distribués
  uniformément sur un disque consiste à simuler des points
  uniformément sur un carré, puis de rejeter ceux qui ne se trouvent
  pas à l'intérieur du disque. La \autoref{fig:simulation:disque}
  illustre cette idée: nous accepterions les points pleins et nous
  rejetterions les points vides.
\end{prob-astuce}
