...
figure~\ref{fig:simulation:acceptation-rejet}, la densité $f$ à
support fini est facilement enveloppée par un rectangle. Un nombre
...
correspondant aux segments en vert, et rejeté dans une proportion
correspondant aux segments en rouge. On voit donc que certaines
valeurs sont rejetées plus souvent que d'autres.
