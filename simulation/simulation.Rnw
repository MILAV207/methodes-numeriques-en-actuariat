...
valeur simulée $y$ de la densité $g$ comme provenant de la densité $f$ si
\begin{equation*}
  u c g_Y(y) \leq f_X(y).
\end{equation*}
Graphiquement, cela signifie que l'on accepte la valeur $y$ si le
point $(y, u c g_Y(y))$ se trouve sous la courbe $f$ en $y$ et qu'on
le rejette s'il se trouve entre $f$ et l'enveloppe. La
...
\end{verbatim}
  ...
  \caption{Code d'une fonction pour simuler des nombres uniformément
    sur un disque de rayon $1$ centré en $0$ par la méthode
    d'acceptation-rejet}
...
@
  ...
  cartésiennes. Si un point du disque unité centré en $0$ se trouve à
  ...
  mise en {\oe}uvre de cette méthode en R. La démonstration de la
  ...
  beaucoup plus rapide que la méthode d'acceptation-rejet:
<<echo=FALSE>>=
...
\end{verbatim}
  \end{framed}
  \caption{Code d'une fonction pour simuler des nombres uniformément
    sur un disque de rayon $1$ centré en $0$ en passant par les
