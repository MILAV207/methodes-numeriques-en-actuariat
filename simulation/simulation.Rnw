...
\begin{objectifs}
\item Être en mesure de développer un algorithme de simulation de
  nombres non uniformes à partir de la méthode de l’inverse.
\item Être en mesure de développer un algorithme de simulation de
  nombres non uniformes à partir de la méthode d’acceptation-rejet.
\item Savoir calculer des nombres pseudo-aléatoires non uniformes en
  suivant un algorithme donné.
\item Connaître et savoir utiliser les outils de Excel, VBA et R pour
  la simulation de nombres non uniformes.
\end{objectifs}
...
section~\ref{sec:simulation:autres}.

Plusieurs algorithmes de simulation de nombres aléatoires non
uniformes reposent sur des résultats de transformations de variables
aléatoires. Pour le lecteur qui ne serait pas à l'aise avec ces
notions, l'annexe~\ref{chap:rappels_transformations} fournit des
rappels sur les principales techniques étudiées dans les cours
d'analyse probabiliste et d'analyse statistique.
...
figure~\ref{fig:simulation:inverse} pour une représentation
...
@
  \caption{Représentation graphique de la méthode de l'inverse. À
    ...
    fonction de répartition à gauche correspond à la densité à droite.}
...
\begin{rem}
  ...
  (section \ref{sec:simulation:excel_et_al}).
\end{rem}
...
\begin{exemple}
  ...
  \begin{gotoR}
    ...
    section~\ref{sec:simulation:code} correspondant à cet exemple
...
la figure~\ref{fig:simulation:discrete} pour une illustration.
...
\begin{exemple}
  ...
  \begin{displaymath}
    ...
    \end{cases}
  \end{displaymath}
  (Voir la figure~\ref{fig:simulation:mixte}.)
...
@
    \caption{Fonction de densité de probabilité (gauche) et fonction de
      répartition (droite) de l'exemple~\ref{exemple:simulation:mixte}}
  ...
  \end{enumerate}
  \begin{gotoR}
    Une mise en oeuvre en R de l'algorithme ci-dessus est présentée
    dans le code informatique de la
    section~\ref{sec:simulation:code}.
  \end{gotoR}
  \qed
\end{exemple}
...
L'idée de la méthode d'acceptation-rejet consiste à accepter la «bonne
proportion» des réalisations de $Y$ comme provenant de $X$. Dans
l'illustration de la figure~\ref{fig:simulation:acceptation-rejet},
la densité $f$ à support fini est facilement enveloppée par un
rectangle. Un nombre simulé de cette distribution (à toutes fins
pratiques une uniforme, ici) est accepté comme provenant de $f$ dans
une proportion correspondant au segments en vert, et rejeté dans une
proportion correspondant au segments en rouge. On voit donc que
certaines valeurs sont rejetées plus souvent que d'autres.
...
\begin{exemple}
  ...
  \ref{fig:simulation:beta-triangle}):
  \begin{figure}
    \centering
<<echo=FALSE, fig=TRUE>>=
...
au tableau \ref{tab:excel}. Les noms anglais des fonctions ont été
...
du tableau \ref{tab:excel} de la propriété \code{WorksheetFunction}.
...
\begin{gotoR}
  Le code informatique R de la section~\ref{sec:simulation:code}
...
\begin{gotoR}
  ...
  informatique R de la section~\ref{sec:simulation:code}.
\end{gotoR}

En terminant, mentionnons que l'annexe~\ref{planification} propose
