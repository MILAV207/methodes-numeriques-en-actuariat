...
L'\autoref{ex:generation:rgl} requiert d'installer le package R
\code{rgl}, disponible sur CRAN%
  \footnote{\url{http://cran.r-project.org}}. %
Tel qu'expliqué à l'annexe~B des notes de cours de la partie~I, on
installe le package dans R avec la commande <<echo=TRUE, eval=FALSE>>=
install.packages("rgl") @ Pour utiliser les fonctions du package, il
faut ensuite charger le package en mémoire avec <<echo=TRUE,
eval=FALSE>>= library(rgl) @
...
\begin{framed}
    ...
    \begin{minipage}[t]{0.85\linewidth}
      \small%
      L'utilisation de \code{rgl} sous Mac OS~X, requiert
      l'application \code{X11.app}, qui n'est plus livrée avec le
      système depuis la version 10.8 (Mountain Lion). Un serveur et
      des bibliothèques client X11 pour OS~X Mountain Lion sont
      disponibles à partir du projet XQuartz. Il suffit de télécharger
      l'application depuis \url{http://xquartz.macosforge.org/}.
    \end{minipage}
  \end{framed}
...
\begin{exercice}
  \begin{enumerate}
  \item Écrire une fonction R faisant la mise en oeuvre du générateur
  ...
  \begin{sol}
    ...
    \item On utilise de nouveau la fonction \code{rand} de
      la \autoref{fig:generation:rand}. Le graphique de la \autoref{fig:generation:a=17} a été créé avec les commandes
...
\begin{exercice}
  ...
  \item Générer une suite $\{x_i\}$ de longueur \nombre{1002} avec le
    ...
    fonction \code{plot3d} du package \code{rgl}, puis faire pivoter
