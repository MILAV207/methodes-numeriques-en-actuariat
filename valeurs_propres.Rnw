...
\label{sec:valeurspropres:valeursetvecteurs}
...
text(w[1], w[2], labels = "(w, z)", pos = 4, cex = 3)
@
    \end{minipage}
  \end{center}
\end{itemize}
...
Graphiquement: lorsque multiplié par $\mat{A}$, un vecteur propre est
transformé en un multiple de lui-même.
\begin{center}
...
\begin{exemple}
  Le vecteur $(1, 2)$ est un vecteur propre de
  \begin{displaymath}
    \mat{A} = \begin{bmatrix} 3 & 0 \\ 8 & -1 \end{bmatrix}
  \end{displaymath}
  correspondant à la valeur propre $\lambda = 3$ car
  \begin{displaymath}
    \begin{bmatrix} 3 & 0 \\ 8 & -1 \end{bmatrix}
    \begin{bmatrix} 1 \\ 2 \end{bmatrix} =
    3 \begin{bmatrix} 1 \\ 2 \end{bmatrix}.
...
\begin{thm}
  ...
  ou inférieure) ou diagonale, alors ses valeurs prores sont les
  éléments de la diagonale.
\end{thm}

Du théorème précédent, on peut établir les faits suivants pour une
matrice triangulaire:
\begin{enumerate}
\item la trace est égale à la somme des valeurs propres;
\item le déterminant est égal au produit des valeurs propres.
\end{enumerate}
Le théorème suivant énonce que ces résultats tiennent pour toute
matrice carrée.
...
\begin{proof}
  ...
  \end{align*}
  Il faut maintenant établir que $c_1 = \tr(\mat{A})$ et que $c_n =
...
\begin{rems}
  \begin{enumerate}
  \item Le théorème ne dit pas que $a_{11}, \dots, a_{nn}$ sont les
    valeurs propres de $\mat{A}$ en général, mais bien seulement que
    \begin{itemize}
    \item $\sum_{i = 1}^n a_{ii} = \text{somme des valeurs propres}$;
    \item $\prod_{i = 1}^n a_{ii} = \text{produit des valeurs propres}$.
    \end{itemize}
  \item Si un élément de la diagonale d'une matrice triangulaire est
    nul, alors
    \begin{itemize}
    \item une valeur propre est nulle;
    \item le déterminant de la matrice est nul et la matrice est
      singulière.
    \end{itemize}
    Cette observation nous permet d'ajouter un énoncé au théorème
    \ref{thm:revision:unification}.
  \end{enumerate}
\end{rems}
...
\begin{thm}
  ...
  \begin{itemize}
  \item $\lambda^k$ est une valeur propre de $\mat{A}^k$;
...
\begin{commentaire}
  Une valeur propre est le nombre de fois qu'il faut «déplacer» la
  matrice $\mat{A}$ de $\mat{I}$ jusqu'à ce qu'elle devienne
  singulière.
\end{commentaire}
...
\begin{itemize}
\item $\mathrm{rang}(\lambda \mat{I} - \mat{A}) < n$;
...
\begin{rems}
  \begin{enumerate}
  \item Le nombre de vecteurs propres dans la base est \emph{au plus}
    la multiplicité de $\lambda$. Par conséquent, si toutes les
    valeurs propres sont différentes, alors on a $n$ vecteurs propres
    différents.
  \item Dans \textsf{R}, les valeurs propres et vecteurs propres sont
    calculés avec la fonction \texttt{eigen}. Les vecteurs propres
...
La diagonalisation d'une matrice est l'application des valeurs et
vecteurs propres qu'un étudiant en actuariat est le plus susceptible
de rencontrer dans ses études de premier cycle.
...
\begin{exemple}
  ...
  efficace, fiable et précise est chose difficile; voir
  \citet{Moler:dubious:1978}.

  Cependant, si $\mat{A}$ est diagonale, alors le calcul est simple.
...
\begin{thm}
  ...
  disctinctes, alors elle est diagonalisable.
\end{thm}
